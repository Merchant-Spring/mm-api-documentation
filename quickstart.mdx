...
resp = requests.post(
    "https://api.marketplacemanager.com/v1/channels",
...
```javascript Node.js
const resp = await fetch("https://api.marketplacemanager.com/v1/channels", {
  method: "POST",
  headers: { "x-api-key": "YOUR_API_KEY", "Content-Type": "application/json" },
  body: JSON.stringify({}),
});
...
```bash
curl -X GET "https://api.marketplacemanager.com/v1/reports/status/YOUR_REPORT_ID" \
  -H "x-api-key: YOUR_API_KEY"
```
